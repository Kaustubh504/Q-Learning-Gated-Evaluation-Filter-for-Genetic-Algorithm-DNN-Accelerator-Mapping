...
class GAMMA(object):
    ...
    def evaluate(self, pool, population, cur_gen=-1, num_elite=0):
        ...
        #
        # The leading `num_elite` individuals in `population` are the elites
        # carried over from last generation (see run(): population = elite +
        # population). They must NEVER be skipped: elitism relies on the
        # guarantee that the population's best-known fitness can't regress
        # generation-to-generation. Letting the filter assign a synthetic
        # -Inf to an elite corrupts that guarantee and demotes the true best
        # genome out of the parent pool for crossover.
        if self.q_filter is not None and cur_gen >= 0:
            protected = [True] * min(num_elite, len(population))
            rest = [self.q_filter.should_evaluate(indv) for indv in population[num_elite:]]
            evaluate_mask = protected + rest
        else:
        ...
        for i in range(len(population)):
            ...
            if self.q_filter is not None and evaluate_mask[i]:
                ...
                if reward[0] == float("-Inf"):
                    self.q_filter.update(state, valid=False)
                else:
                    self.q_filter.update(state, reward=reward[0], valid=True)
            # ────────────────────────────────────────────────────────────────
    ...
    def run(self, dimension, stage_idx=0, prev_stage_value=0, num_population=100, num_generations=100, elite_ratio=0.05,
        ...
        for g in range(num_generations):
            ...
            chkpt = self.evaluate(pool=pool, population=population, cur_gen=g, num_elite=len(elite))
